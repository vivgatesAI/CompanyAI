...
function buildInstruction({ presetKey, customPrompt }) {
  ...
  return `${likenessGuardrail} ${styleTarget}`;
}
...
app.post("/api/edit", upload.single("image"), async (req, res) => {
  try {
    ...
    const prompt = buildInstruction({ presetKey, customPrompt });

    const payload = {
      image: req.file.buffer.toString("base64"),
      prompt,
      modelId: process.env.VENICE_IMAGE_EDIT_MODEL || "qwen-edit",
